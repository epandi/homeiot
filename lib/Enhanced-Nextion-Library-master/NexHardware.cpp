...
void sendCommand(const char* cmd)
{
    ...
    nexSerial.print(cmd);
    // EP: Debug the command to send
    dbSerialPrint("sendCommand: ");
    dbSerialPrintln(cmd);
...
bool nexInit(const uint32_t baud)
{
    ...
    // EP: HARDWARE SERIAL2 RX 16; TX 17
    ...
    // nexSerial.begin(9600, SERIAL_8N1, 16, 17); // default baud, it is recommended that do not change defaul baud on Nextion, because it can forgot it on re-start
    ...
    if(baud!=9600)
    {
        ...
        //nexSerial.begin(baud, SERIAL_8N1, 16, 17);
    }
